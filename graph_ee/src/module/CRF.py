...
class CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        decode_idx_look = decode_idx.tolist()
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        if new_tags.max() >= tag_size * tag_size:
            print('new_tags bigger')
        elif new_tags.min() < 0:
            print('new_tags smaller')


        """ need convert tags id to search from 400 positions of scores """
        tg_energy = torch.gather(scores.view(seq_len, batch_size, -1), 2, new_tags).view(seq_len, batch_size)
        mask_transpose = mask.transpose(1, 0)
        tg_energy = tg_energy.masked_select(mask_transpose)
        # tg_energy = torch.masked_select(tg_energy, mask_transpose)

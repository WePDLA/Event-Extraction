...
import sys
import json
from module.utils import read_by_lines, write_by_lines
        ...
        for line in f:
            ...
            if is_predict:
                ...
                if model == "trigger":
                    ...
                    count_event_i = 0
                    labels = ["O"] * len(text_a)
                    # 把这句话放在这里是为了让同一句话中的不同事件的事件元素能够在同一条数据中体现，起到了set的作用
                    for event in d_json["event_list"]:
                        count_event_i += 1
                        if count_event_i > 1:
                            count += 1
                            # print('event_list:',d_json["event_list"])
                        # labels = ["O"] * len(text_a)
...
if __name__ == "__main__":
    ...
    # data process
    data_dir = "../../data/DuEE"
    trigger_save_dir = "{}/trigger_set".format(data_dir)
    role_save_dir = "{}/role_set".format(data_dir)
    print("\n=================start schema process==============")

...
from module.trig_dp_graph_model import Trig_dp_graph_Model
...
tedian = 'ace-1type-sentemb-char-loop-qiao'
# tedian = 'ace-1type-char-loop'
...
#arg parms
do_arg_train_mark = False
do_arg_predict_mark = False

if do_train_mark:
    print("do trigger train")
...
def convert_example_to_feature(example, tokenizer, label_vocab=None, max_seq_len=max_len, no_entity_label="O", ignore_label=-1, is_test=False):
...
class EE_Dataset(torch.utils.data.Dataset):
    def __init__(self, encodings, labels, senq_len_list, sent_dp_vec_list):
        self.encodings = encodings
        self.labels = labels
        self.senq_len = senq_len_list
        self.sent_dp_vec_list = sent_dp_vec_list
        # print('done')
    ...
    def __getitem__(self, idx):
        ...
        item['sent_dp_vec'] = torch.tensor(self.sent_dp_vec_list[idx])
        return item

    def __len__(self):
        return len(self.labels)

def encode_tags(tags, encodings, trig_label2id, sent_dp_vec_list, train_texts=[], special_char_list = []):
    labels = [[trig_label2id[tag] for tag in doc] for doc in tags]
    encoded_labels = []
    senq_len_list = []
    sent_dp_vec_return = []
    count = 0
    for doc_labels, doc_offset, text, token_ids, sent_dp_vec in zip(labels, encodings.offset_mapping,
                                train_texts, encodings.input_ids, sent_dp_vec_list):
        # create an empty array of -100
        if len(doc_labels) > max_len - 2:
        ...
        # 处理sent_dp_vec, 由于Dependency_Parsing文件中已经规定了max_seq_len最大的长度是512，且已经考虑了首尾两个特殊字符
        # 因此，这里要做的就是将sent_dp_vec扩展到批处理的最大长度
        sent_dp_new = [np.zeros([len(doc_offset),len(doc_offset)], dtype=bool) for i in range(len(sent_dp_vec))]
        sent_dp_size = sent_dp_vec[0].shape[-1]
        relation_types_num = len(sent_dp_vec)
        for rel_type_j in range(relation_types_num):
            sent_dp_new[rel_type_j][:sent_dp_size, :sent_dp_size] = sent_dp_vec[rel_type_j]
        ...
        sent_dp_vec_return.append(np.array(sent_dp_new))
        count += 1
        del sent_dp_vec
    return encoded_labels, special_char_list, senq_len_list,sent_dp_vec_return

def load_DataAndLabelDict(data_file, data_id2label, data_label2id ,tokenizer,
                          dataset_type='not_train', special_char_list=[], is_predict=False):
    ...
    data_encodings = tokenizer(data_texts, is_split_into_words=True,
            return_offsets_mapping=True, padding=True, truncation=True, max_length=max_len)
    # 获取依存关系的向量表示
    sents_dp_vec_file = tmp_file + dataset_type + '_'  + tedian + '_' +'.pkl'
    # if os.path.exists(sents_dp_vec_file):
    try:
        sent_dp_vec_list = joblib.load(sents_dp_vec_file)

    except:
        sent_dp_vec_list = sent_dp.get_pars_vec(data_texts)
        joblib.dump(sent_dp_vec_list, sents_dp_vec_file)

    # 对标签进行编码（主要是将其补充到与tokenizer文本一样的长度）
    data_encoded_labels, special_char_list, senq_len_list,sent_dp_vec_list = encode_tags(data_labels, data_encodings, data_label2id,
                    sent_dp_vec_list, data_texts, special_char_list=special_char_list)
    train_dataset = EE_Dataset(data_encodings, data_encoded_labels, senq_len_list, sent_dp_vec_list)
    if is_predict:
        shuffle = False
    else:
        shuffle = True
...
def do_trig_train(train_data_file,dev_data_file,test_data_file,dict_file,dict_save_file,
                  dee_model_name, model_type='bert_liner'):
    # load datasets
    ...
    data_label2id, data_id2label, dev_loader, special_char_list = load_DataAndLabelDict(dev_data_file,
                data_id2label, data_label2id, tokenizer, dataset_type='dev',
                                        special_char_list=special_char_list)
    ...
    if model_type.lower().split('_')[-1] == 'liner':
        ...
        model = Trig_BertCRF_Model(MODEL_PATH, tokenizer, label_num,
                hidden_size=hidden_size,device= device,label2id = data_label2id)
    ...
    elif model_type.lower().split('_')[-1] == 'gcn':
        model = Trig_dp_graph_Model(MODEL_PATH, tokenizer, label_num,
                label2id = data_label2id, device= device)
    else:
...
def do_trig_predict(test_data_file,pred_sents_file, dict_file,dict_save_file, dee_model_name,predict_save_path):
    ...
    data_label2id, data_id2label, test_loader, special_char_list = load_DataAndLabelDict(test_data_file,
        data_id2label, data_label2id, tokenizer, special_char_list=special_char_list, is_predict=True)
...
if __name__ == '__main__':
    ...
    if do_train_mark or do_predict_mark:
        ...
        trig_data_dir = data_dir + 'trigger_set/'
    ...
    if do_arg_train_mark or do_arg_predict_mark:
        ...
        arg_data_dir = data_dir + 'role_set/'
    ...
    if do_train_mark:
        do_trig_train(trig_train_data_file,trig_dev_data_file,trig_test_data_file,
                      trig_dict_file,trig_dict_save_file,dee_trig_model_name,model_type)
    elif do_predict_mark:
        do_trig_predict(trig_test_data_file,pred_sents_file, trig_dict_file,
            trig_dict_save_file, dee_trig_model_name, trig_predict_save_path)

    if do_arg_train_mark:
        do_trig_train(arg_train_data_file, arg_dev_data_file,arg_test_data_file,
                      arg_dict_file,arg_dict_save_file, dee_arg_model_name, model_type)
    elif do_arg_predict_mark:
        do_trig_predict(arg_test_data_file,pred_sents_file, arg_dict_file,
                        arg_dict_save_file, dee_arg_model_name, arg_predict_save_path)
